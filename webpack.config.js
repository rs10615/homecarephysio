...
module.exports = (env) => {
	...
	return {
		...
		output: {
			...
			filename: isDev ? 'js/[name].bundle.js' : 'build/[name].[hash].bundle.js',
			sourceMapFilename: isDev ? 'js/[name].bundle.map' : 'build/[name].[chunkhash].bundle.map',
			chunkFilename: isDev ? 'js/[id].chunk.js' : 'build/[id].[chunkhash].chunk.js',
			publicPath: './'
		},
		module: {
			rules: [
