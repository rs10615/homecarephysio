...
export default class SmartMenuItem extends React.Component {
	...
	scrollTo(item, event) {
		if (item.route === '/') {
			...
			var domElement = document.getElementById(item.scrollTo);
			if (domElement) {
	...
	render() {
		...
		if (item.scrollTo) {
			link = <a onClick={this.scrollTo.bind(this, item)} title={item.title}>
				{icon} {title} {badge}
			</a>
		}

		return item.isLogoImg ? <li className='logo'><a ></a></li> :
			<li className={liClassName}>{link}{childItems}</li>
	}
}
